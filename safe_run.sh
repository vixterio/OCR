...
echo "[watchdog] baseline swap=${BASE_SWAP}MB  limits: rss<${RSS_LIMIT_MB}MB swap<+${SWAP_GROWTH_MB}MB free>${FREE_PCT_MIN}%"
...
while kill -0 "$CHILD" 2>/dev/null; do
  ...
  if [ "$SW_DELTA" -gt "$SWAP_GROWTH_MB" ]; then REASON="swap grew ${SW_DELTA}MB > ${SWAP_GROWTH_MB}MB"; fi
